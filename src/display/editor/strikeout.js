 ...
 */
class StrikeoutEditor extends AnnotationEditor {
  #boxes;
  ...
  constructor(params) {
    ...
    StrikeoutEditor._defaultColor ||=
      this._uiManager.highlightColors?.values().next().value || "#fff066";
    this.color = params.color || StrikeoutEditor._defaultColor;
  ...
  #createStrikeouts() {
    ...
    this.#highlightStrikeouts = outliner.getUnderlinesAndStrikeouts();
    ({
      x: this.x,
      y: this.y,
      width: this.width,
      height: this.height,
    } = this.#highlightStrikeouts.box);

    // The last point is in the pages coordinate system.
    const { lastPoint } = this.#highlightStrikeouts.box;
    this.#lastPoint = [
  ...
  static updateDefaultParams(type, value) {
    switch (type) {
      case AnnotationEditorParamsType.HIGHLIGHT_COLOR:
        StrikeoutEditor._defaultColor = value;
        break;
    }
  }
  ...
  get propertiesToUpdate() {
    return [
      [
        AnnotationEditorParamsType.HIGHLIGHT_COLOR,
        this.color || StrikeoutEditor._defaultColor,
      ],
    ];
  }
  ...
  #addToDrawLayer(parent = this.parent) {
    ...
    ({ id: this.#id } = parent.drawLayer.strikeout(
      this.#highlightStrikeouts,
      this.color,
      this.#opacity
    ));
  }
  ...
  rotate(angle) {
    ...
    drawLayer.updateBox(this.#id, StrikeoutEditor.#rotateBbox(this, angle));
  }
  ...
  #serializeStrikeouts() {
    const [pageWidth, pageHeight] = this.pageDimensions;
    const underlineBoxes = this.#highlightStrikeouts.lineRects;
    const box = this.#highlightStrikeouts.box;
  ...
  serialize(isForCopying = false) {
    ...
    return {
      ...
      strikeouts: this.#serializeStrikeouts(),
